...
class BuildZoneVi(
    private val buildZoneContext: Context,
    private val buildZoneCallback: BuildZoneCallBack,
    private val buildZoneWindow: Window
) : WebView(buildZoneContext) {
    private var buildZoneFileChooserHandler: ((ValueCallback<Array<Uri>>?) -> Unit)? = null
    fun buildZoneSetFileChooserHandler(handler: (ValueCallback<Array<Uri>>?) -> Unit) {
        this.buildZoneFileChooserHandler = handler
    }
    init {
        ...
        webSettings.apply {
            ...
            userAgentString = WebSettings.getDefaultUserAgent(buildZoneContext).replace("; wv)", "").replace("Version/4.0 ", "")
        ...
        super.setWebViewClient(object : WebViewClient() {
            override fun shouldOverrideUrlLoading(
                ...
                return if (request?.isRedirect == true) {
                    ...
                    try {
                        buildZoneContext.startActivity(intent)
                    } catch (e: Exception) {
                        Toast.makeText(buildZoneContext, "This application not found", Toast.LENGTH_SHORT).show()
                    }
                    true
                }
            }
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                if (url?.contains("https://test-web.syndi-test.net") != true && url?.contains("https://web.team-s.club") != true) {
                    BuildZoneApplication.buildZoneInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN
                    Log.d(BuildZoneApplication.BUILD_ZONE_MAIN_TAG, "onPageFinished : ${BuildZoneApplication.buildZoneInputMode}")
                    buildZoneWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);
                    buildZoneEnableStableInputScroll()
                } else {
                    BuildZoneApplication.buildZoneInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE
                    Log.d(BuildZoneApplication.BUILD_ZONE_MAIN_TAG, "onPageFinished : ${BuildZoneApplication.buildZoneInputMode}")
                    buildZoneWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
                }
            }
        ...
        super.setWebChromeClient(object : WebChromeClient() {
            ...
            override fun onShowFileChooser(
                ...
                buildZoneFileChooserHandler?.invoke(filePathCallback)
                return true
            }
            override fun onCreateWindow(
                ...
                buildZoneHandleCreateWebWindowRequest(resultMsg)
                return true
            }
        })
    }
    ...
    fun buildZoneFLoad(link: String) {
        super.loadUrl(link)
    }

    private fun buildZoneEnableStableInputScroll() {
        evaluateJavascript(
            """
        (function() {
    ...
    private fun buildZoneHandleCreateWebWindowRequest(resultMsg: Message?) {
        if (resultMsg == null) return
        if (resultMsg.obj != null && resultMsg.obj is WebView.WebViewTransport) {
            val transport = resultMsg.obj as WebView.WebViewTransport
            val windowWebView = BuildZoneVi(buildZoneContext, buildZoneCallback, buildZoneWindow)
            transport.webView = windowWebView
            resultMsg.sendToTarget()
            buildZoneCallback.buildZoneHandleCreateWebWindowRequest(windowWebView)
        }
    }
